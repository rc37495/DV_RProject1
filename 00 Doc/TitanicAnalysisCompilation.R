...
ggplot() + 
  ...
  layer(data=df, 
        mapping=aes(x=SEX, y=as.numeric(as.character(FARE)), color=factor(SURVIVED)), 
        stat="identity", 
        stat_params=list(), 
        geom="point",
        geom_params=list(), 
...
ggplot() + 
  ...
  scale_y_continuous() +
  facet_wrap(~PCLASS) +
  labs(title='Combined Effect of Class and Offspring on Titanic Embarkment') +
  labs(x="Port of Origin", y=paste("Number of Offspring")) +
  layer(data=df, 
        mapping=aes(x= EMBARKED, y=as.numeric(as.character(PARCH)), color=EMBARKED), 
        stat="identity", 
        stat_params=list(), 
        geom="point",
        geom_params=list(), 
